...
\begin{abstract}
    ...
    hardware's future input.  However, embodied situations
    are one of the primary potential uses of neuromorphic hardware.  To address
    this, we present a methodology for generating embodied benchmarks that makes use
...
or could be learned using some back-propogation of error method.  Here, for simplicity, we only
...
In addition, we add a external perturbing forces.  In a real system,
...
particular piece of hardware, using it to control a minimal simulation, and
measure some aspect of that simulation.  We perform this test multiple times,
...
This is shown in Figure \ref{fig:analysis_compute}.  In this case, the CPU can
run XXX neurons, the GPU XXX neurons, and a single SpiNNaker chip can run XXX
neurons. Furthermore, we also show the effects of increasing the number
...
efficiency.  For this reason, the third benchmark fixes the number of neurons
at however many that hardware can compute in 0.5 Watts.  While this is a
reasonable number for SpiNNaker, neither the CPU nor the GPU can even run
with that little power.  For this reason, we simply take the number of
neurons that hardware can simulate in real time and scale it by the power
consumption of the hardware.  The result is that the CPU can run XXX neurons,
the GPU XXX neurons, and SpiNNaker can run XXX neurons.  The results of
this benchmark are shown in Figure \ref{fig:analysis_power}.  This clearly
...
we show what happens as the delays $\t_q$ and $\t_u$ are increased.
...
task.  Further analysis is needed to determine exactly why this is.
